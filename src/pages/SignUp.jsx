...
export default function SignUp() {
  ...
  async function onSubmit(e) {
    ...
    try {
      ...
      await setDoc(doc(db, "users", user.uid), formDataCopy); // saved data into database
      navigate("/"); // redirect to home page

    } catch (error) {
      toast.error("Something went wrong with the registration. Please try again later.");
    }
  }
    ...
    <section>
      ...
      <div className='flex justify-center flex-wrap items-center px-6 py-12 max-w-6xl mx-auto'>
        <div className='md:w-[67%] lg:w-[50%] mb-12 md:mb-6'>
          <img src="https://images.unsplash.com/flagged/photo-1564767609342-620cb19b2357?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1073&q=80" alt="sign-in" className='w-full rounded-2xl' />
        </div>
        <div className='w-full md:w-[67%] lg:w-[40%] lg:ml-20'>
          <form onSubmit={onSubmit}>
            <input type="text" id='name' value={name} onChange={onChange} placeholder='your name' className='w-full px-4 py-2 text-xl text-gray-700 bg-white border-gray-300 rounded transition ease-in-out mb-6' />
            ...
            <div className='flex justify-between whitespace-nowrap text-sm sm:text-lg'>
              <p className='mb-6 '>Have a account?<Link to="/sign-in" className='text-red-600 hover:text-red-600 transition duration-200 ease-in-out ml-1'>Sign In</Link></p>
